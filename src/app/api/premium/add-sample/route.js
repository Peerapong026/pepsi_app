...
export async function POST(req) {
  try {
    ...
    if (!body.pm_id_premium || body.pm_type_premium !== "ของชิม") {
      return NextResponse.json({ message: "ข้อมูลไม่ครบหรือประเภทไม่ถูกต้อง" }, { status: 400 });
    }

    await Premium.create(body);
    return NextResponse.json({ message: "บันทึกของชิมสำเร็จ" }, { status: 201 });
  } catch (error) {
    console.error("❌ POST /add-sample error:", error);
